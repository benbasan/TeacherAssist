...
│   └── OddOneOut.tsx           # Social-deduction empathy sim ("odd one out"/חרם): detective leaves, class adopts 1 of 14 secret rules → amber rule reveal + 3 examples → 2-min interrogation floor (3 guesses) → calm indigo/lavender empathy debrief, NO confetti (setup→reveal→interrogation→reflection; §4a)
├── pages/
